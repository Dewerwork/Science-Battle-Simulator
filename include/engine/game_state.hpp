...
// Movement constants kept for reference, but actual values from MovementCalculator
constexpr u8 STANDARD_MOVE = 6;
constexpr u8 FAST_MOVE = 9;       // Fast: +3" movement (was 8, corrected)
constexpr u8 SLOW_MOVE = 4;       // Slow: -2" movement
...
struct GameState {
    ...
    // Movement helpers - delegate to MovementCalculator
    u8 get_move_speed(const Unit& unit) const {
        // Use registry-based movement calculation
        return MovementCalculator::calculate_move_distance(
            unit, nullptr, MovementContext::MoveType::ADVANCE);
    }

    // Calculate effective charge distance based on unit rules
    u8 get_charge_distance(const Unit& unit) const {
        // Use registry-based charge calculation
